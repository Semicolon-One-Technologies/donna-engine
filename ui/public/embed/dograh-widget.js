/**
 * Donna Engine Voice Widget
 * Embeddable voice call widget for Donna Engine workflows
 * Version: 1.0.0
 */
...
(function() {
  ...
  async function init() {
    ...
    state.config = {
      ...
      contextVariables: parseContextVariables(script.getAttribute('data-donna-engine-context'))
    };
  ...
  function injectStyles() {
    ...
    const styles = `
      ...
      .donna-engine-widget-cta {
        ...
        animation: donna-engine-cta-in 220ms ease-out;
      }
      ...
      .donna-engine-widget-cta.donna-engine-state-connecting { background: #f59e0b !important; animation: donna-engine-pulse 1.6s infinite; }
      .donna-engine-widget-cta.donna-engine-state-connected  { background: #ef4444 !important; }
      .donna-engine-widget-cta.donna-engine-state-failed     { background: #ef4444 !important; opacity: 0.85; }

      @keyframes donna-engine-pulse {
        0%, 100% { opacity: 1; }
        50% { opacity: 0.6; }
      }

      @keyframes donna-engine-cta-in {
  ...
  function renderFloating() {
    ...
    button.type = 'button';
    button.className = `donna-engine-widget-cta donna-engine-state-${status}`;
  ...
  function updateInlineStatus(status, text, subtext) {
    ...
    if (status === 'idle' || status === 'failed') {
      ...
      buttonHTML = `
        <button class="donna-engine-inline-btn donna-engine-inline-btn-start" id="donna-engine-inline-start-btn" style="background: ${state.config.buttonColor};">
      ...
      // Red button to end
      buttonHTML = `
        <button class="donna-engine-inline-btn donna-engine-inline-btn-end" id="donna-engine-inline-end-btn">
          End Call
        </button>
      `;
    }
    ...
    container.innerHTML = `
      <div class="donna-engine-inline-status">
        ...
        <p class="donna-engine-inline-status-text">${displayText}</p>
        <p class="donna-engine-inline-status-subtext">${displaySubtext}</p>
