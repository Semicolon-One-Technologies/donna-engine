                ...
                const response = await createOrUpdateEmbedTokenApiV1WorkflowWorkflowIdEmbedTokenPost({
                    ...
                    body: {
                        ...
                        settings: {
                            ...
                            size: "medium",
                            autoStart: false,
                            containerId: embedMode === "inline" ? "donna-engine-inline-container" : undefined,
                        },
                        usage_limit: null,
                                        ...
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                ...
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    ...
                                                    <li>• Call <code className="text-xs">window.DonnaEngineWidget.start()</code> to begin a call.</li>
                                                    <li>• Call <code className="text-xs">window.DonnaEngineWidget.end()</code> to end it.</li>
...
window.DonnaEngineWidget?.onStatusChange((status) => {
  callStatus = status;
...
document.getElementById('talk-btn').addEventListener('click', () => {
  if (callStatus === 'connected' || callStatus === 'connecting') {
    window.DonnaEngineWidget.end();
  } else {
    window.DonnaEngineWidget.start();
  }
});`}</code>
                                                </pre>
  ...
  useEffect(() => {
    window.DonnaEngineWidget?.onStatusChange(setStatus);
  }, []);

  const isLive = status === 'connected' || status === 'connecting';
  return (
    <button onClick={() => isLive ? window.DonnaEngineWidget.end() : window.DonnaEngineWidget.start()}>
                                    ...
                                    {embedMode === "inline" && (
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                <h4 className="font-medium mb-2">Integration Instructions</h4>
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    <li>• Add a div with id=&quot;donna-engine-inline-container&quot; where you want the widget</li>
                                                    ...
                                                    {widgetType === "chat" ? (
                                                        ...
                                                        <>
                                                            <li>• Call window.DonnaEngineWidget.start() to begin the call</li>
                                                            <li>• Call window.DonnaEngineWidget.end() to end the call</li>
                                                        </>
                                                    )}
                                                </ul>
                                            </div>
...
<div id="donna-engine-inline-container" style="min-height: 480px">
  <!-- Chat panel renders here; no extra JS needed -->
                                                ...
                                                <div className="rounded-lg bg-blue-50 dark:bg-blue-950/20 p-4 border border-blue-200 dark:border-blue-800">
                                                    ...
                                                    <pre className="text-xs overflow-x-auto">
                                                        <code className="text-blue-800 dark:text-blue-200">{`export function DonnaEngineAgent() {
  const [isCallActive, setIsCallActive] = useState(false);

  useEffect(() => {
    // Widget will auto-initialize when script loads
    window.DonnaEngineWidget?.onCallStart(() => {
      setIsCallActive(true);
    });
    window.DonnaEngineWidget?.onCallEnd(() => {
  ...
  return (
    <div className="my-8">
      ...
      <div id="donna-engine-inline-container" className="min-h-[400px]">
        {/* Widget renders here */}
      </div>
      <button
        onClick={() => window.DonnaEngineWidget?.start()}
