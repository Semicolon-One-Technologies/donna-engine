---
title: "Update"
description: "Update your self-hosted Donna stack to a newer image version"
---

This guide covers updating a Donna stack you've already deployed with [Docker](/deployment/docker) or a [custom domain](/deployment/custom-domain). You run commands from the same directory that contains your `docker-compose.yaml` (this is the `donna/` directory if you used `setup_remote.sh`).
...
Donna publishes two images — `donna-api` and `donna-ui` — to Docker Hub:

- **Docker Hub** — [hub.docker.com/u/semicolononetechnologies](https://hub.docker.com/u/semicolononetechnologies)

Each release is published under two kinds of tags. Note the formats differ between releases and the Docker image tags — `update_remote.sh` understands both and normalizes for you.

| Where | Tag format | Example | When to use |
|-------|-----------|---------|-------------|
| Release tag | `donna-vX.Y.Z` | `donna-v1.28.0` | Stable versioned releases |
...
<Warning>
Always update **`donna-api`** and **`donna-ui`** to the **same tag**. The two images are built from the same commit and the UI expects API responses in a matching shape — mixing versions will break the app. `update_remote.sh` handles this for you automatically.
</Warning>
...
- Synchronizes the canonical remote keys in `.env` and validates the runtime config that `donna-init` will render from it.
...
```bash
cd donna
curl -o update_remote.sh https://raw.githubusercontent.com/semicolononetechnologies/donna-engine/main/scripts/update_remote.sh
bash update_remote.sh
```

You'll be prompted for the target version, defaulting to the most recent release. Accepted forms: bare semver (`1.28.0`), v-prefixed (`v1.28.0`), the full tag (`donna-v1.28.0`), or `main` for bleeding edge — the script normalizes them. Non-interactive callers can set it via environment variable and skip the confirmation prompt:

```bash
TARGET_VERSION=1.28.0 DONNA_UPDATE_YES=1 bash update_remote.sh
```
...
```bash
cd donna
for f in docker-compose.yaml nginx.conf turnserver.conf .env remote_up.sh scripts/run_donna_init.sh scripts/lib/setup_common.sh deploy/templates/nginx.remote.conf.template deploy/templates/turnserver.remote.conf.template; do
...
<Warning>
Rolling back across a database migration is not always safe — if the newer release ran a schema migration, downgrading may leave the DB in a state the older API doesn't understand. If in doubt, contact [Donna support](https://support.speaktodonna.com) before rolling back.
</Warning>
...
# Track latest main:
git pull
# Or pin to a specific release (git tag format is donna-vX.Y.Z):
git checkout donna-v1.28.0
