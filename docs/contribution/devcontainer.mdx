...
description: Details for running and maintaining the Donna devcontainer.
---

The checked-in `.devcontainer/` follows the Dev Containers specification, so it should also work in Cursor, JetBrains IDEs, and other Dev Container-compatible editors. Only the VS Code path is actively tested. If something breaks elsewhere, please contact [Donna support](https://support.speaktodonna.com).
