...
Donna is an open-source, full-stack voice agent platform: a Next.js UI, a FastAPI backend, Postgres/Redis/MinIO for storage, and a [Pipecat](https://github.com/pipecat-ai/pipecat)-based pipeline (checked in as a git submodule) that drives the actual voice conversation.
...
| `sdk/` | Python/TypeScript SDKs for driving Donna programmatically |
...
If you cloned `Semicolon-One-Technologies/donna-engine` directly instead of forking first, see [Fork Workflow](/contribution/fork-workflow) to fix up your `origin`/`upstream` remotes.
...
4. Push to your fork (`origin`) and open a pull request.
5. A maintainer reviews and merges.

Bug reports and feature ideas go through [GitHub Issues](https://github.com/Semicolon-One-Technologies/donna-engine/issues). Issues tagged `good first issue` are a good place to start.
